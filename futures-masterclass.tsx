...
const Store = {
  async load() {
    try {
      const r = await window.storage.get('ftm-v3-progress');
      return r ? JSON.parse(r.value) : null;
    } catch { return null; }
  },
  async save(data) {
    try { await window.storage.set('ftm-v3-progress', JSON.stringify(data)); } catch {}
  }
};
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg0,color:C.tx,fontFamily:"'Inter',system-ui,sans-serif",overflowY:"auto"}}>
      <div style={{background:`linear-gradient(135deg,${C.bg1},#0A1A35)`,borderBottom:`1px solid ${C.bd}`,padding:"60px 32px 48px",textAlign:"center"}}>
        ...
        <div style={{color:C.gold,fontSize:11,fontFamily:"monospace",letterSpacing:".2em",marginBottom:10,textTransform:"uppercase"}}>Complete Futures Course</div>
        <h1 style={{fontSize:36,fontWeight:900,margin:"0 0 14px",lineHeight:1.1,background:`linear-gradient(90deg,${C.gold},${C.goldL})`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Futures Trading<br/>Masterclass</h1>
      ...
      {sideOpen&&(
        <div style={{width:268,background:C.bg1,borderRight:`1px solid ${C.bd}`,display:"flex",flexDirection:"column",flexShrink:0,overflow:"hidden"}}>
          <div onClick={()=>{setShowHome(true);save(completed,modIdx,lesIdx,true);}} style={{padding:"12px 14px",borderBottom:`1px solid ${C.bd}`,cursor:"pointer"}}>
            <div style={{color:C.gold,fontSize:13,fontWeight:800}}>📈 FuturesMasterClass</div>
